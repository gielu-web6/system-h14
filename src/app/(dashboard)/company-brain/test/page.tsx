    ...
    <div className="max-w-[900px] space-y-5">
      <div>
        <h1 className="text-[20px] font-bold text-fg flex items-center gap-2">
          <TestTube size={20} className="text-pink" /> Tester kontekstu AI
        </h1>
      ...
      <div className="card-elevated rounded-[14px] p-5 space-y-4">
        <div className="space-y-1.5">
          <label className="text-[11px] font-semibold text-muted uppercase tracking-wide">Wybierz featurę</label>
          <select
            value={feature}
        ...
        <div className="space-y-1.5">
          <label className="text-[11px] font-semibold text-muted uppercase tracking-wide">Zapytanie testowe</label>
          {selectedFeature && (
            <p className="text-[11px] text-subtle italic">Przykład: {selectedFeature.hint}</p>
          )}
          <textarea
            value={query}
      ...
      {result && (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold ${result.dna_present ? 'bg-success/15 text-success' : 'bg-danger/15 text-danger'}`}>
              <Brain size={12} /> DNA: {result.dna_present ? '✓ Dostępne' : '✗ Brak'}
            </span>
            <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold ${result.chunks_count > 0 ? 'bg-pink/15 text-pink' : 'bg-fg/[0.07] text-muted'}`}>
              <Layers size={12} /> {result.chunks_count} fragmentów z plików
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold bg-fg/[0.07] text-muted">
              ~{result.token_estimate} tokenów kontekstu
            </span>
          </div>

          {/* Tabs */}
          <div className="flex gap-1 bg-raised border border-border rounded-[10px] p-1">
            {(['context', 'chunks', 'gaps'] as const).map(t => (
              <button key={t}
                onClick={() => setActiveTab(t)}
                className={`flex-1 px-3 py-1.5 rounded-[7px] text-[12px] font-medium transition-all ${
                  activeTab === t ? 'bg-pink shadow-[var(--glow-pink)]' : 'text-subtle hover:text-muted'
                }`}
                style={activeTab === t ? { color: 'var(--nav-pill-text)' } : undefined}
              >
          ...
          {activeTab === 'context' && (
            <div className="card-elevated rounded-[14px] overflow-hidden">
              <div className="px-5 py-3 border-b border-border">
                <p className="text-[13px] font-semibold text-fg">Kontekst który trafi do AI</p>
                <p className="text-[11px] text-muted">Dokładnie to co system wstrzyknie na początku prompta</p>
              </div>
              <div className="p-5">
                <pre className="text-[11px] text-muted font-mono leading-relaxed whitespace-pre-wrap overflow-auto max-h-[500px] bg-raised rounded-[8px] p-3">
            ...
            <div className="space-y-3">
              {result.chunks.length === 0 ? (
                <div className="text-center py-8 text-muted">
                  <Layers size={24} className="mx-auto mb-2 text-subtle" />
                  <p className="text-[13px]">Brak fragmentów — dodaj pliki kontekstowe i przetworz je</p>
                </div>
              ) : (
                result.chunks.map((chunk, i) => (
                  <div key={chunk.id} className="card-elevated rounded-[12px] overflow-hidden">
                    <button
                      onClick={() => setExpandedChunk(expandedChunk === chunk.id ? null : chunk.id)}
                      className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-fg/[0.03] transition-colors"
                    >
                      <span className="text-[11px] font-bold text-pink w-5 flex-shrink-0">#{i + 1}</span>
                      <div className="flex-1 min-w-0">
                        {chunk.content_summary && (
                          <p className="text-[12px] font-medium text-fg truncate">{chunk.content_summary}</p>
                        )}
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] text-subtle">{chunk.category}</span>
                          <span className="text-[10px] font-semibold text-success">{Math.round(chunk.similarity * 100)}% match</span>
                        </div>
                      </div>
                      {expandedChunk === chunk.id ? <ChevronUp size={13} className="text-subtle" /> : <ChevronDown size={13} className="text-subtle" />}
                    </button>
                    {expandedChunk === chunk.id && (
                      <div className="px-4 pb-4 border-t border-border-s">
                        <pre className="text-[11px] text-muted font-mono leading-relaxed whitespace-pre-wrap mt-3 max-h-[300px] overflow-auto bg-raised rounded-[8px] p-3">
                          {chunk.content}
                        </pre>
                      </div>
                    )}
            ...
            <div className="space-y-3">
              {result.gaps.length === 0 ? (
                ...
                result.gaps.map((gap, i) => (
                  <div key={i} className="flex items-start gap-3 p-3.5 rounded-[10px] bg-amber/[0.07] border border-amber/20">
                    <AlertTriangle size={14} className="text-amber flex-shrink-0 mt-0.5" />
                    <p className="text-[12px] text-muted leading-snug">{gap}</p>
                  </div>
                ))
              )}
            </div>
