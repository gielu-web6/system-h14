      ...
      <div className="bg-card border border-border rounded-[14px] p-5">
        ...
        <div className="flex items-start gap-0 overflow-x-auto pb-1">
          {MESSAGE_TYPE_OPTIONS.map((t, i, arr) => {
            ...
            const color = isActive ? '#E8A838' : '#374151'
            return (
              <div key={t.id} className="flex items-start flex-shrink-0">
                ...
                >
                  <div
                    ...
                    style={{ background: isActive ? '#E8A838' : 'rgba(255,255,255,0.06)', color: isActive ? '#000' : 'rgba(255,255,255,0.35)' }}
                  >
                    {i + 1}
                  </div>
                  <p className={`text-[10px] font-bold text-center leading-tight transition-colors ${isActive ? 'text-[#E8A838]' : 'text-white/40 group-hover:text-white/70'}`}>{t.label.split(' — ')[0]}</p>
                  <p className={`text-[9px] text-center mt-0.5 ${isActive ? 'text-[#E8A838]/70' : 'text-white/25'}`}>{t.label.split(' — ')[1] ?? ''}</p>
                  <span className={`text-[9px] font-semibold mt-1 px-1.5 py-0.5 rounded-full ${isActive ? 'bg-[#E8A838]/15 text-[#E8A838]' : 'bg-white/[0.06] text-white/30'}`}>{t.day}</span>
                </button>
                {i < arr.length - 1 && (
